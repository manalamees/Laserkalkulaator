...
# Salvesta faili metainfo (nimi, indeks) session_state'i, et rerun ei kaotaks faile mobiilis.
if "uploaded_file_meta" not in st.session_state:
    st.session_state["uploaded_file_meta"] = {}  # file_key -> (idx, name)

if uploaded_files:
    for idx, uploaded in enumerate(uploaded_files):
        file_key = uploaded_file_key(idx, uploaded)
        if file_key not in st.session_state["uploaded_file_meta"]:
            st.session_state["uploaded_file_meta"][file_key] = (idx, uploaded.name)

# Eemalda kustutatud võtmed metainfost
for deleted_key in st.session_state["deleted_upload_keys"]:
    st.session_state["uploaded_file_meta"].pop(deleted_key, None)
    st.session_state.get("uploaded_file_bytes", {}).pop(deleted_key, None)

# Ehita active_uploaded_files session_state'ist — töötab ka pärast mobiili rerun'i
class _FakeUploaded:
    """Asendab UploadedFile objekti kui brauser ei saada faili uuesti."""
...
cached_bytes = st.session_state.get("uploaded_file_bytes", {})
cached_meta = st.session_state.get("uploaded_file_meta", {})

# Kasuta päris uploaded objekti kui olemas, muidu session_state cache'ist
if uploaded_files:
    for idx, uploaded in enumerate(uploaded_files):
        file_key = uploaded_file_key(idx, uploaded)
    ...
    # Mobiil tegi rerun — taasta session_state'ist
    for file_key, (idx, name) in cached_meta.items():
        if file_key not in st.session_state["deleted_upload_keys"]:
            file_bytes = cached_bytes.get(file_key)
            if file_bytes:
                active_uploaded_files.append((idx, file_key, _FakeUploaded(name, file_bytes)))
